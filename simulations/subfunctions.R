...
###############################################################################################
cum_bsln_hzd = function(surv.fit, rel_hzd=NULL, t_star, lambda=F){
  rel_hzd = rel_hzd
  ...
  if(is.null(rel_hzd))  rel_hzd = exp(model.matrix(surv.fit)%*%c(surv.fit$coefficients))
  dat$rel_hzd = rel_hzd
  dat$wt=1;
...
Delta_mu_3m = function(Lambda0, Lambda0.all, t, t1, tau1, x, betas){
  Delta_mu_tau1 = Delta_mu_inf(Lambda0=Lambda0, t=t[t<=tau1], t1=t1, x=x, betas=betas[,1:2])
  nbeta = nrow(betas)
  rr0 = exp(cbind(x, 0)%*%betas[,1:2]); 
  Lmd12.0 = t(t(rr0)*Lambda0[tau1,])
  Lmd1.1 = Lambda0[tau1,1]*rr0[,1]*exp(betas[nbeta,1])
  Lmd.0 = outer(c(Lambda0[t>=tau1]), c((exp(cbind(x, 0)%*%betas[,-(1:2)]))))
  Delta_mu_tau = rbind(Delta_mu_tau1$Delta_mu,
